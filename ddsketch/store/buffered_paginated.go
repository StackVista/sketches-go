...
// of the page is less than the memory space that is needed to keep in the
// buffer the indexes that would otherwise be encoded in that page. That process
// (transferring indexes from the buffer to pages) is called compaction. That
// means that some indexes may stay indefinitely in the buffer if, to be removed
// from the buffer, they would create a page that is almost empty.
// This store never collapses or merges bins, therefore, does not introduce any
// error in itself. In particular, MinIndex(), MaxIndex(), Bins() and
// KeyAtRank() return exact indexes.
...
type BufferedPaginatedStore struct {
	...
	bufferCompactionTriggerLen int   // compaction happens only after this buffer length has been reached
...
// in the buffer than the new page, and possibly, the extension of the top-level
// slice, take.
...
// on varying input distributions may however ultimately make the store overly
// large and may waste memory space (because of empty pages or a buffer with
// large capacity).
